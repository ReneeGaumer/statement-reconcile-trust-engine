...
Every export request must receive exactly one final export classification.
...
EXPORT_EMBARGO is a hard stop on certified export, release, or trust-certified downstream use.



EXPORT_EMBARGO does not prohibit diagnostic evaluation, exception reporting, reconciliation analysis, evidence review, decision explanation, or audit package generation.



When EXPORT_EMBARGO is assigned, the platform must still produce sufficient diagnostic records to explain why export certification failed, unless source data cannot be safely processed at all.



Every trust determination must receive exactly one final trust classification.
...
A failed Courtroom Test shall not suppress diagnostic findings.



If a trust determination cannot be certified, the system must explain the failure through authoritative records, exceptions, evidence lineage, and decision rationale.



The absence of certifiable trust is itself a trust-relevant conclusion and must be recorded.
...
Article 14 - Diagnostic Output and Export Certification



The platform distinguishes diagnostic outputs from certified exports.



Diagnostic outputs include exception reports, reconciliation findings, evidence sufficiency findings, decision explanations, trust failure reports, and audit reconstruction packages.



Certified exports are derived artifacts released as trust-supported outputs.



EXPORT_EMBARGO prohibits certified export and release.



EXPORT_EMBARGO does not prohibit diagnostic outputs whose purpose is to explain, remediate, or reconstruct trust failure.



Diagnostic outputs generated under EXPORT_EMBARGO must be clearly labeled as not trust-certified for downstream use.



No embargoed diagnostic output may be represented as a CLEAN_EXPORT, EXPORT_WITH_WARNINGS, PARTIAL_EXPORT, or UNSAFE_EXPORT certified export.
